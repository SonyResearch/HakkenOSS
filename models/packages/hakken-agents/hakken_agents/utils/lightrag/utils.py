    ...
    def final_decro(func):
        ...
        async def shutdown():
            ...
            try:
                ...
                logger.warning(f"{queue_name}: Timeout waiting for queue to empty during shutdown")
...
async def handle_cache(
    ...
    if mode != "default":  # handle cache for all type of query
        if not hashing_kv.global_config.get("enable_llm_cache"):
            return None
    elif not hashing_kv.global_config.get("enable_llm_cache_for_entity_extract"):
        return None
...
def sanitize_text_for_encoding(text: str, replacement_char: str = "") -> str:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"Text sanitization failed with unexpected error: {e!s}") from e
