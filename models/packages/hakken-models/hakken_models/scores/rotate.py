...
class RotatEScore(ScoreFn):
    ...
    def get_complex_dim(self, embeddings: torch.Tensor) -> int:
        return embeddings.shape[1] // 2

    def _to_complex(self, embeddings: torch.Tensor) -> torch.Tensor:
    ...
    def _normalize_relation_embeddings(self, r_emb: torch.Tensor) -> torch.Tensor:
    ...
    def _rotate_score(
        self, h_emb: torch.Tensor, r_emb: torch.Tensor, t_emb: torch.Tensor
    ) -> torch.Tensor:
        ...
        return cast(torch.Tensor, -distance)
