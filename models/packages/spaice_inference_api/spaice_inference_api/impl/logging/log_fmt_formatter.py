...
if TYPE_CHECKING:
    from spaice_inference_api.core.contract.logger import PossibleLabelValues
...
LABEL_KEYS_ENABLED = os.getenv("SPAICE_LOG_LOGFMT_LABEL_KEYS_ENABLED", "1") in [
    "1",
    "True",
    "true",
]
...
class LogFmtFormatter(Logfmter):
    ...
    def __init__(
        ...
        if keys is None:
            keys = ["at"]
        self.common_labels.set(common_labels)
        self.possible_label_values.set(
            {
                **self.__process_possible_label_values(possible_label_values),
                **{"level": {"max_length": 8}},
            }
        )
        if datefmt is not None:
            super().__init__(keys, mapping, datefmt)
        else:
            super().__init__(keys, mapping)
