...
class NdjsonReferenceReader(ReferenceReader[NdjsonReferenceReaderConfig]):
    def __init__(self, config: NdjsonReferenceReaderConfig):
    ...
    def iter_publications(self, num_skips: int = 0) -> "Iterator[Publication]":
        ...
        file_paths = sorted(
            p
            for p in self.config.publications_directory.iterdir()
            if p.suffix in accepted_extensions
        )

        for file_path in file_paths:
            df = pd.read_json(file_path, lines=True).replace({pd.NA: None, np.nan: None})
            ...
            for _, row in df.iterrows():
                ...
                try:
                    ...
                    logger.info(
                        f"Met KeyError while processing {pmid}, key: {key}, file: {file_path}; "
                        f"skipping the row"
                    )
                except ValueError as e:
                    logger.info(
                        f"Got error while processing {pmid}, file: {file_path}; skipping the row"
                    )
                    logger.info(str(e))
    ...
    def iter_publication_concept_links(
        ...
        file_paths = sorted(
            p
            for p in self.config.publication_concept_links_directory.iterdir()
            if p.suffix in accepted_extensions
        )
