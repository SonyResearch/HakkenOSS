...
class RDF2VecTransformer:
    ...
    def fit(self, kg: KG, entities: Entities, is_update: bool = False) -> RDF2VecTransformer:
        ...
        if self.verbose >= 1:
            ...
            if len(self._walks) != len(walks):
                ...
                logging.info(f"> {n_walks} walks extracted for {len(self._entities)} entities.")
        return self
    ...
    def get_walks(self, kg: KG, entities: Entities) -> List[List[SWalk]]:
        ...
        # if not kg.skip_verify:
        #     missing_entities = [entity for entity in entities if not kg.is_exist(entity)]

        #     if missing_entities:
        #         # If running in an asynchronous context, close the connector if needed.
        #         if kg.mul_req:
        #             asyncio.run(kg.connector.close())

        #         # Raise an error and print missing entities
        #         missing_entities_str = ", ".join(missing_entities)
        #         raise ValueError(f"At least one provided entity does not exist in the Knowledge Graph: {missing_entities_str}")
        ...
        if self.verbose >= 1:
            n_walks = sum([len(entity_walks) for entity_walks in walks])
            print(
                f"Extracted {n_walks} walks " + f"for {len(entities)} entities ({toc - tic:0.4f}s)"
            )
